      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <CardContent class="space-y-4">
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p class="text-gray-900 mt-1">{{ contact?.phone || 'Not provided' }}</p>
                </div>
                <div>
                  <label class="text-sm font-medium text-gray-700">Company</label>
                  <p class="text-gray-900 mt-1">{{ contact?.company || 'Individual' }}</p>
                </div>
                <div>
                  <label class="text-sm font-medium text-gray-700">Source</label>
                  <p class="text-gray-900 mt-1 capitalize">{{ contact?.source || 'Unknown' }}</p>
                </div>
                <div>
          ...
          <Card>
            ...
            <CardContent>
              <div class="space-y-4">
                <div v-for="history in contactHistory" :key="history.id" class="flex items-start space-x-3 p-3 bg-gray-50 rounded-lg">
                  <div class="flex-shrink-0">
                    <div class="w-8 h-8 bg-blue-100 rounded-full flex items-center justify-center">
                      <svg v-if="history.type === 'email'" class="w-4 h-4 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 4.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                      </svg>
                      <svg v-else-if="history.type === 'call'" class="w-4 h-4 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <div class="flex-1">
                    <div class="flex items-center justify-between">
                      <h4 class="text-sm font-medium capitalize">{{ history.type }} {{ history.direction }}</h4>
                      <span class="text-xs text-gray-500">{{ formatDate(history.date) }}</span>
                    </div>
                    <p class="text-sm text-gray-600 mt-1">{{ history.description }}</p>
                  </div>
                </div>
                <div v-if="contactHistory.length === 0" class="text-center py-4 text-gray-500">
        ...
        <div class="space-y-6">
          ...
          <Card>
            ...
            <CardContent class="space-y-4">
              <div class="flex justify-between">
                ...
                <span class="font-medium">{{ contact?.projectsCount || 0 }}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-sm text-gray-600">Total Value</span>
                <span class="font-medium">MYR {{ (contact?.totalValue || 0).toLocaleString() }}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-sm text-gray-600">Client Since</span>
                <span class="font-medium">{{ formatDate(contact?.createdAt) }}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-sm text-gray-600">Last Contact</span>
                <span class="font-medium">{{ contact?.lastContactDate ? formatDate(contact.lastContactDate) : 'Never' }}</span>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div class="space-y-3">
                <div v-for="project in recentProjects" :key="project.id" class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  <div>
                    <h4 class="text-sm font-medium">{{ project.name }}</h4>
                    <p class="text-xs text-gray-500">{{ formatDate(project.date) }}</p>
                  </div>
                  <Badge :variant="project.status === 'completed' ? 'default' : 'outline'" class="text-xs">
                    {{ project.status }}
                  </Badge>
                </div>
                <div v-if="recentProjects.length === 0" class="text-center py-4 text-gray-500 text-sm">
                  No projects yet.
                </div>
              </div>
            </CardContent>
...
const loadContact = async () => {
  ...
  try {
    ...
    contact.value = {
      ...
      notes: "High-value client, prefers premium packages. Very detail-oriented and has specific requirements for lighting and angles.",
    };
  } catch (error) {

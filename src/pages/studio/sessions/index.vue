<template>
  <div class="space-y-6">
    ...
    <div class="space-y-6">
      ...
      <!-- Session Types Table View -->
      <div v-else-if="sessionTypes.length > 0 && viewMode === 'table'">
        <DataTable
          :data="sessionTypes"
      ...
      <!-- Session Types Card View -->
      <div
        v-else-if="sessionTypes.length > 0 && viewMode === 'card'"
      ...
      >
        ...
        >
          <CardContent class="p-5">
            ...
            >
              <div>
                Shooter Price: MYR {{ sessionType.shooterPrice.toFixed(2) }}
              </div>
        ...
        <Card
          ...
          class="border border-dashed border-primary/30 hover:shadow-sm hover:border-primary/50 transition-all duration-200 cursor-pointer flex items-center justify-center h-[210px]"
        >
          <CardContent class="flex flex-col items-center text-center">
            ...
            <h4 class="font-medium text-foreground">Add New Session</h4>
          </CardContent>
        </Card>
      </div>
      ...
      <div
        v-else
        class="text-center py-16 border-2 border-dashed border-primary/30 rounded-xl bg-primary/5"
      >
        <div class="mb-6">
          <div class="p-4 bg-primary/10 inline-flex rounded-full mx-auto mb-2">
            <CameraIcon class="w-8 h-8 text-primary" />
          </div>
...
const toggleSessionStatus = async (session: SessionType) => {
  try {
    ...
    // await sessionTypeService.updateSessionType(session.id, { isActive: !session.isActive });
